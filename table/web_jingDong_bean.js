...
async function main() {
  ...
  */
  ...
  const getUserInformation = async (url) => {
    ...
    const response = await getCacheString('info.json', url, 'POST', headers);
    return response.base;
  };

  const { headImageUrl, nickname, jdNum, jvalue } = await getUserInformation('https://wq.jd.com/user/info/QueryJDUserInfo?sceneval=2');
  ...
  const Run = async () => {
    if (randomIndex === 0) {
      ...
      val = {
        ...
        text2: `明天加 ${tomorrowSendBeans} 京豆`,
        lightColor: '#000000',
        darkColor: '#FFA500'
      }
