...
async function main() {
  ...
  */
  ...
  if (Fm.fileExists(maybach)) {
    maybachData = JSON.parse(Fm.readString(maybach));
  }
  ...
  async function createWidget() {
    ...
    */
    ...
    // update icon
    const upTimeStack = leftStack.addStack();
    upTimeStack.layoutHorizontally();
    upTimeStack.centerAlignContent();
    if (nothing) {
      const iconSymbol2 = SFSymbol.named('timer');
      const carIcon2 = upTimeStack.addImage(iconSymbol2.image);
      carIcon2.imageSize = new Size(16, 16);
      carIcon2.tintColor = Color.black();
      upTimeStack.addSpacer(5);
    }
      
    // update time
    const updateTime = upTimeStack.addStack();
    const textUpdateTime = updateTime.addText(nothing ? maybachData.parkingTime : `${vio.violationTime}`);
    textUpdateTime.font = Font.mediumSystemFont(13);
    textUpdateTime.textColor = new Color('#484848');
    leftStack.addSpacer(nothing ? 22 : 5)
    ...
    */
    ...
    const textAddress = tipsStack.addText(nothing ? maybachData.address : `${vio.violationAddress}，` + `${vio.violation}`);
